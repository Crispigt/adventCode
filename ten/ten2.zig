...
fn findPaths(mat: std.ArrayList([]i32) ) !i32 {
    var x: usize = 0;
    var y: usize = 0;
    var res: i32 = 0;
    // 
    ...
    for (mat.items) |value| {
        for (value) |val| {
            if(val == 0){
                ...
                res += try findNextNumber(x, y, 1, mat);
...
fn findNextNumber(x: usize, y: usize, next:i32, mat: std.ArrayList([]i32)) anyerror!i32 {
    //Need to check so that not going outside inside the box
    //std.debug.print("{d},{d}\n", .{x,y});
    std.debug.print("x: {d}, y: {d}, {d}\n", .{x,y, next});

    var cur: i32 = 0;
    cur += try up(x, y, next, mat);
    cur += try down(x, y, next, mat);
    cur += try left(x, y, next, mat);
    cur += try right(x, y, next, mat);
    
    return cur;
}

fn up(x: usize, y: usize, next: i32, mat: std.ArrayList([]i32)) !i32 {
    if (y <= 0) return 0;
    ...
    if (mat.items[y-1][x] == next) {
        ...
        return try findNextNumber(x, y-1, next + 1, mat);
    }
    return 0;
}

fn down(x: usize, y: usize, next: i32, mat: std.ArrayList([]i32)) !i32 {
...
fn right(x: usize, y: usize, next: i32, mat: std.ArrayList([]i32)) !i32 {
    ...
    if (mat.items[y][x+1] == next) {
        ...
        return try findNextNumber(x+1, y, next + 1, mat);
    }
    return 0;
}

fn left(x: usize, y: usize, next: i32, mat: std.ArrayList([]i32)) !i32 {
    if (x <= 0) return 0;
    // Correct indexing: mat.items[y][x-1]
    if (mat.items[y][x-1] == next) {
        if (next == 9 ) {
...
pub fn main() !void {
    ...
    const res = try findPaths(array1);
    std.debug.print("{d}\n", .{res});
}
